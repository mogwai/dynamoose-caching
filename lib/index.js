...
const getStrategy = strategy => require('./strategies/' + strategy);
...
function cacheGet(model, cache) {
  ...
  model.get = function(key, options) {
    if (options && options.ignoreCache) return old.apply(this, arguments);
    console.log();
    const hash = model.$__.name + key; 
...
function cacheOp(model, optype, cache) {
  ...
  model[optype] = function() {
    ...
    op.exec = function(ignoreCache) {
      ...
      const hash = model.name + hashO(this.options) + hashO(this.filters);
      debug(this.options, this.filters);
      if (cache.has(hash)) {
        debug('Cache hit');
        return cache.get(hash);
      }
